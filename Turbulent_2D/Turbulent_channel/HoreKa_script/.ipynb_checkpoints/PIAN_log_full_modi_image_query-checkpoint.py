...
def Inner_filter(x,y,c):
    ...
    height=dist_repo[c_new,position_l]+(dist_repo[c_new,position_r]-dist_repo[c_new,position_l])*(x_new-x_corr[position_l])/(x_corr[position_r]-x_corr[position_l])
    return (y[...,0]>height)
inner_sampler = tp.samplers.RandomUniformSampler(Sim_domain, n_points=2000,filter_fn=Inner_filter).make_static(resample_interval=100)#,filter_fn=Inner_filter)
...
def Log_filter(x,y,c):
    return (y[...,0]>y_lim)
log_sampler = tp.samplers.RandomUniformSampler(Sim_domain_sub_log, n_points=1000,filter_fn=Log_filter)#.make_static(resample_interval=200)#,filter_fn=Inner_filter)
...
class FCN_model_Fourier_Feature_CNN(nn.Module):
    def __init__(self,input_space,output_space,N_features,sigma_1=1,sigma_2=15):
        ...
        self.fc1_l=nn.Linear(in_features=N_features,out_features=500,device=GPU)
        self.fc2_l=nn.Linear(in_features=500,out_features=500,device=GPU)
        self.fc3_l=nn.Linear(in_features=500,out_features=500,device=GPU)
        ###
        self.fc1_r=nn.Linear(in_features=N_features,out_features=500,device=GPU)
        self.fc2_r=nn.Linear(in_features=500,out_features=500,device=GPU)
        self.fc3_r=nn.Linear(in_features=500,out_features=500,device=GPU)


        self.conv1=nn.Conv1d(1,30,7,stride=2,padding=3,device=GPU) #((720-7+2*3)/2)+1 *8=720 *5
        self.act1=nn.LeakyReLU(0.1)
        self.pool1=nn.MaxPool1d(kernel_size = 3, stride = 2, padding = 1)# stride=2   (720-2)/2 + 1 *8 = 360 *3
        
        self.conv2=nn.Conv1d(30,60,3,stride=1,padding=1,device=GPU) # ((720-3+2*1)/1)+1 *32 = 720*9
        self.act2=nn.LeakyReLU(0.1)
        self.pool2=nn.MaxPool1d(kernel_size = 3, stride = 2, padding = 1) #360*6

        self.conv3=nn.Conv1d(60,128,5,stride=3,padding=1,device=GPU) #((720-5+2*1)/3)+1 *32 = 240*15
        self.act3=nn.LeakyReLU(0.1)
        self.pool3=nn.MaxPool1d(kernel_size = 3, stride = 2, padding = 1) #360*6
        self.conv4=nn.Conv1d(128,256,5,stride=3,padding=1,device=GPU) #((120-5+2*1)/3)+1 *32 = 40*20
        self.act4=nn.LeakyReLU(0.1)
        self.pool4=nn.MaxPool1d(kernel_size = 3, stride = 2, padding = 1) #360*6

        self.flat=nn.Flatten()
        
        self.fc_combo1=nn.Linear(in_features=1768,out_features=2048,device=GPU)
        self.fc_combo2=nn.Linear(in_features=2048,out_features=2048,device=GPU)
        self.fc_combo3=nn.Linear(in_features=2048,out_features=2048,device=GPU)
        self.fc_combo4=nn.Linear(in_features=2048,out_features=2048,device=GPU)
        self.fc_combo5=nn.Linear(in_features=2048,out_features=2048,device=GPU)
        self.fc_combo6=nn.Linear(in_features=2048,out_features=2048,device=GPU)


        
        self.out=nn.Linear(in_features=2048,out_features=output_space.dim,device=GPU)
...
model=FCN_model_Fourier_Feature_CNN(input_space=X*Y,output_space=U*V*URMS*VRMS*UV*P,N_features=1000)
#model=torch.load("PIAN_log_sum_logarithmic_full.pt")
model.train()
...
#disc=torch.load("Critic_log_sum_logarithmic_80*50_full.pt")
disc.train()
...
def log_diagnostic(u,y,c):
    ...
    return tp.utils.grad(u,y)/(800*utau)*(y-torch.mean(dist_repo[c_new,:],dim=1)[:,None])*800-2.6
pde_cond_log=tp.conditions.PINNCondition_CNN(model, log_sampler, log_diagnostic, dist_matrix=dist_repo,weight=500,name='logarithmic')
...
optim_G = tp.OptimizerSetting(torch.optim.Adam, lr=0.00005,scheduler_class=torch.optim.lr_scheduler.ReduceLROnPlateau,scheduler_args={"patience":10000,"factor":0.8,"verbose":True,"min_lr":0.000005},monitor_lr="train/model_loss")
optim_D = tp.OptimizerSetting(torch.optim.Adam, lr=0.00005,scheduler_class=torch.optim.lr_scheduler.ReduceLROnPlateau,scheduler_args={"patience":10000,"factor":0.8,"verbose":True,"min_lr":0.000005},monitor_lr="train/D_loss")
...
torch.save(model,"PIAN_log_sum_logarithmic_full_checkpoint1.pt")
torch.save(disc,"Critic_log_sum_logarithmic_80_50_full_checkpoint1.pt")
