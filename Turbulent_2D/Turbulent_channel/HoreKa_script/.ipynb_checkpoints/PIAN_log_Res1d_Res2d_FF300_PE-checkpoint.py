...
Data_folders=["Round0","Round1","Round05","Round1_AL","Round2_AL","Round3_AL","Round4_AL","Round_sparse","Round_snap"]
DF_Data=pd.read_csv("Data/Database_PINN/"+Data_folders[0]+"/Flow_3d_mesh_6_X720Y226_half_plus.csv") # DNS flow data
for i_folder in Data_folders[1:]:
    DF_temp=pd.read_csv("Data/Database_PINN/"+i_folder+"/Flow_3d_mesh_6_X720Y226_half_plus.csv") # DNS flow data
    DF_temp["c"]=DF_temp["c"]+1+max(DF_Data["c"])
...
DF_DIST=pd.read_csv("Data/Database_PINN/"+Data_folders[0]+"/dist_single.csv",header=None)  # raw roughness topographies without flipping
x_corr=DF_DIST.iloc[0].to_numpy() # the x coordinate
dist_repo=DF_DIST.iloc[1:].to_numpy()
DF_DIST=pd.read_csv("Data/Database_PINN/"+Data_folders[0]+"/dist_odd.csv",header=None) # raw roughness topographies with flipping
dist_repo=np.vstack((dist_repo,DF_DIST.iloc[1:].to_numpy()))
for i_folder in Data_folders[1:]:
    try:
        DF_DIST=pd.read_csv("Data/Database_PINN/"+i_folder+"/dist_single.csv",header=None)
        dist_repo=np.vstack((dist_repo,DF_DIST.iloc[1:].to_numpy()))
    except:
        pass
    try:
        DF_DIST=pd.read_csv("Data/Database_PINN/"+i_folder+"/dist_odd.csv",header=None)
        dist_repo=np.vstack((dist_repo,DF_DIST.iloc[1:].to_numpy()))
    except:
        pass
        
x_corr_new=np.linspace(min(x_corr),max(x_corr),N_x) # coarser coordinate for critic
dist_repo_low=np.zeros((len(dist_repo),N_x)) # repository of roughness topographies lower resolution for critic 
...
N_c_train=len(dist_repo) # total number of available raw topographies 

X_interval = tp.domains.Interval(X, 0, L_x) # the prediciton x range of PIAN generator
Y_interval = tp.domains.Interval(Y, 0, 1.0) # from 0 to channel half height 1
C_interval = tp.domains.Interval(C,0,N_c_train) # number of available raw topographies

Sim_domain = X_interval*Y_interval*C_interval # PIAN generator domain


Y_interval_sub_low=tp.domains.Interval(Y, 0, 0.15)     # only covers roughness region for faster sampling 
Sim_domain_sub_low=X_interval*Y_interval_sub_low*C_interval

Y_interval_sub_log=tp.domains.Interval(Y,y_lim,1)      # only covers regions outside the sliding window for applying log diagnostic plot
Sim_domain_sub_log=X_interval*Y_interval_sub_log*C_interval

N_c=len(DF_Data["c"].unique()) # number of available training data
DF_vincinity=DF_Data[(DF_Data.y<y_lim)] # truncate dns data below y_lim, i.e. within the height of sliding window


Data_Pinn=torch.permute(torch.tensor(DF_vincinity[["U","V","urms","vrms","uv"]].to_numpy(),dtype=torch.float32,device=GPU).reshape((N_c,N_x,N_y_sub,5)),(0,3,1,2)) # transform 1-d data two 4-d [case_number, x_position, y_position,flow_field_components (U,V,Urms,Vrms,UV)]



#eta=torch.cos(torch.pi*(torch.tensor([i for i in range(N_y*2)],requires_grad=False,device=GPU))/(N_y*2-1)) 

ys=DF_vincinity["y"].unique()[:N_y_sub] # use y grid points as in dns (i.e. with stretching)
ys=torch.tensor(ys,dtype=torch.float32,device=GPU)

DF_DIST=pd.read_csv("Data/Database_PINN/"+Data_folders[0]+"/dist_train.csv",header=None) # the roughness topographies corresponding to the DNS data, this should be 1-to-1 matching with the DF_Data
x_corr=DF_DIST.iloc[0].to_numpy()
dist=DF_DIST.iloc[1:].to_numpy()
for i_folder in Data_folders[1:]:
        DF_DIST=pd.read_csv("Data/Database_PINN/"+i_folder+"/dist_train.csv",header=None)
        dist=np.vstack((dist,DF_DIST.iloc[1:].to_numpy()))
    
x_corr_new=np.linspace(min(x_corr),max(x_corr),N_x) # get coarse roughness profile to fit Critic resolution
...
matrix_mask_Data=(ymesh<dist_new.reshape((N_c,1,N_x,-1)))|(ymesh>=(2-dist_new.reshape((N_c,1,N_x,-1)))) # 0-1 mask for distinguishing roughness and fluid 
matrix_mask_Data=matrix_mask_Data.long()


def IBM_irr_filter_low(x,y,c): # see torchphysics tutorial -> this filter let the sampler only select points within roughnesss
    ...
    return (y[...,0]<=height)
IBM_sampler_irr_low = tp.samplers.RandomUniformSampler(Sim_domain_sub_low,n_points=1000,filter_fn=IBM_irr_filter_low).make_static(resample_interval=1000) # uniform sampler is used here with incorporation of the roughness filter function above
...
def Inner_filter(x,y,c):# get sampling points above roughenss 
    position_r=torch.tensor([sum(x_corr<x_i)-1 for x_i in x],device=GPU)
...
def Log_filter(x,y,c):# get sampling points exclusively outside the sliding window area
    return (y[...,0]>y_lim)
...
def self_sin(input):  # customized sin activation
    return input.sin()
def self_cos(input): # customized cos activation
    return input.cos()
...
### Construction of 1d residual net
...
class ResNet1d(nn.Module):
    def __init__(self, block, PE_depth,layers,input_space,output_space,N_features,sigma_1=1,sigma_2=15):
        super(ResNet1d, self).__init__()
        self.inplanes = 64
        self.conv1 = nn.Sequential(
                        nn.Conv1d(PE_depth, 64, kernel_size = 7, stride = 2, padding = 3),
                        nn.BatchNorm1d(64),
                        nn.ReLU())
        ...
        for k in torch.arange(720):
            for i in torch.arange(int(PE_depth/2)):
                denominator=np.power(1000,2*i/PE_depth)
                PE[0,2*i,k]=torch.sin(k/denominator)
                PE[0,2*i+1,k]=torch.cos(k/denominator)
        self.PE=torch.tensor(PE,requires_grad=False)
    ...
    #x = self.avgpool(x)
        ...
        t=t.as_tensor[:,0:2] ##only take X and Y, ignore C
...
model=ResNet1d(ResidualBlock_1d,[2,2,2,2],PE_depth=8,input_space=X*Y,output_space=U*V*URMS*VRMS*UV*P,N_features=300).to(GPU)       
#model=torch.load("PIAN_log_sum_logarithmic_new_long.pt")
#model.train()
...
#disc=torch.load("Critic_log_sum_logarithmic_80*50_new.pt")
#disc.train()

## please refer to torchphysics tutorial for the codes below
def pde_IBM(u,v): #No-slip condition roughness
...
pde_cond_log=tp.conditions.PINNCondition_CNN(model, log_sampler, log_diagnostic, dist_matrix=dist_repo,weight=100,name='logarithmic')




def pde_mass(u,v,x,y):#conti
...
def pde_residual_x(u,v, x, y,urms,vrms,uv,p,c): # momentum x
    c_new=torch.transpose(c,1,0)[0]
    c_new = c_new.to(torch.long)
...
def pde_residual_y(u,v, x, y,urms,vrms,uv,p): # momentum y
...
def boundary_residual(u,v,urms,vrms,uv,p, x,y,c): # boundary conditions
...
optim_G = tp.OptimizerSetting(torch.optim.Adam, lr=0.00001,scheduler_class=torch.optim.lr_scheduler.ReduceLROnPlateau,scheduler_args={"patience":10000,"factor":0.8,"verbose":True,"min_lr":0.000005},monitor_lr="train/model_loss")
optim_D = tp.OptimizerSetting(torch.optim.Adam, lr=0.00001,scheduler_class=torch.optim.lr_scheduler.ReduceLROnPlateau,scheduler_args={"patience":10000,"factor":0.8,"verbose":True,"min_lr":0.000005},monitor_lr="train/D_loss")
...
list_of_Losses=[         pde_cond_log,
                pde_cond_IBM_low,
                           ...
                           pde_cond_y,
                pde_cond_mass]
solver = tp.solver.PIAN_Solver_CNN_Wasserstein_LowMem_half_logarithmic(list_of_Losses,
                               generator=model,
                               discriminator=disc,
...
trainer = pl.Trainer(accelerator='gpu',# use one GPU
                     devices=[0,1,2,3], # Distributed Data Parallel
                     max_steps=150000, # iteration number
                     benchmark=True,
                     logger=comet_logger, # for writting into tensorboard
                     log_every_n_steps=100,# write log file
...
torch.save(model,"PIAN.pt")
torch.save(disc,"Critic.pt")
