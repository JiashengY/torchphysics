...
Data_folders=["Round_snap"]
DF_Data=pd.read_csv("Data/Database_PINN/"+Data_folders[0]+"/Flow_3d_mesh_6_X720Y226_half_plus.csv")
#len(DF_Data[(DF_Data.c==0)&(DF_Data.y<0.4)]["y"].unique())
for i_folder in Data_folders[1:]:
    DF_temp=pd.read_csv("Data/Database_PINN/"+i_folder+"/Flow_3d_mesh_6_X720Y226_half_plus.csv")
...
DF_DIST=pd.read_csv("Data/Database_PINN/"+Data_folders[0]+"/dist_single.csv",header=None)
x_corr=DF_DIST.iloc[0].to_numpy()
dist_repo=DF_DIST.iloc[1:].to_numpy()
DF_DIST=pd.read_csv("Data/Database_PINN/"+Data_folders[0]+"/dist_odd.csv",header=None)
...
DF_DIST=pd.read_csv("Data/Database_PINN/"+Data_folders[0]+"/dist_train.csv",header=None)
x_corr=DF_DIST.iloc[0].to_numpy()
...
class FCN_model_Fourier_Feature_CNN(nn.Module):
    def __init__(self,input_space,output_space,N_features,sigma_1=1,sigma_2=15,PE_depth=8):
        super().__init__()
        ...
        self.conv1=nn.Conv1d(PE_depth,10,3,stride=1,padding=1,device=GPU) #((720-3+2*1)/1)+1 *8=720 *5
        self.act1=nn.LeakyReLU(0.1)
        #self.pool1=nn.MaxPool1d(kernel_size=2) # stride=2   (720-2)/2 + 1 *8 = 360 *3

        self.conv2=nn.Conv1d(10,15,3,stride=1,padding=1,device=GPU) # ((720-3+2*1)/1)+1 *32 = 720*9
        self.act2=nn.LeakyReLU(0.1)
        #self.pool2=nn.MaxPool1d(kernel_size=2) #360*6

        self.conv3=nn.Conv1d(15,18,5,stride=3,padding=1,device=GPU) #((720-5+2*1)/3)+1 *32 = 240*15
        self.act3=nn.LeakyReLU(0.1)
        #self.pool3=nn.MaxPool1d(kernel_size=2)# 120*9
        self.conv4=nn.Conv1d(18,20,5,stride=3,padding=1,device=GPU) #((120-5+2*1)/3)+1 *32 = 40*20
    ...
    def forward(self,dist_1d,t):
        ...
        t_figure=dist_1d.expand(-1,8,-1)
        t_figure=t_figure+self.PE
        t_figure=self.act1(self.conv1(t_figure))
...
optim_G = tp.OptimizerSetting(torch.optim.Adam, lr=0.0001,scheduler_class=torch.optim.lr_scheduler.ReduceLROnPlateau,scheduler_args={"patience":30000,"factor":0.8,"verbose":True,"min_lr":0.000005},monitor_lr="train/model_loss")
optim_D = tp.OptimizerSetting(torch.optim.Adam, lr=0.0001,scheduler_class=torch.optim.lr_scheduler.ReduceLROnPlateau,scheduler_args={"patience":30000,"factor":0.8,"verbose":True,"min_lr":0.000005},monitor_lr="train/D_loss")
...
trainer = pl.Trainer(accelerator='gpu',# use one GPU
                     devices=[0,1,2,3],
                     max_steps=300000, # iteration number
