...
N_x=144 # grid in x
N_y=91 # grid in y
N_dists=32 # N 1d roughness for fake images during training
...
matrix_mask_Data=(ymesh<dist_new.reshape((N_c,1,N_x,-1)))
...
def IBM_irr_filter_up(x,y,c):
    ...
    height=dist[c_new,position_l]/100+(dist[c_new,position_r]-dist[c_new,position_l])*(x_new-x_corr[position_l])/(x_corr[position_r]-x_corr[position_l])
    return (y[...,0]>=2-height)   
...
class Discriminator(nn.Module):
    def __init__(self):
        super().__init__()
        # 300 * 100
        self.conv1=nn.Conv2d(6,12,3,stride=1,padding=1,device=GPU) #((91-3+2*1)/1)+1 * ((144-3+2*1)/1)+1 *18 =91 * 144 
        self.act1=nn.LeakyReLU(0.1)
        self.pool1=nn.MaxPool2d(kernel_size=2,stride=2) # stride=2   (45-2)/2 + 1  * (72-2)/2 + 1 *18 =  45 72

        self.conv2=nn.Conv2d(12,12,3,stride=1,padding=1,device=GPU) #((23-3+2*1)/1)+1 * ((36-3+2*1)/1)+1 *32 =45 * 72 *32
        self.act2=nn.LeakyReLU(0.1)
        self.pool2=nn.MaxPool2d(kernel_size=2,stride=2) # stride=2   (150-2)/2 + 1  * (50-2)/2 + 1 *18 = 22 * 36  *12

        self.conv3=nn.Conv2d(12,12,5,stride=3,padding=1,device=GPU) #((22-5+2*1)/3)+1 * ((36-5+2*1)/3)+1 *18 =7 * 12 *12
        self.act3=nn.LeakyReLU(0.1)
        self.pool3=nn.MaxPool2d(kernel_size=2,stride=2) # stride=2   (21-2)/2 + 1  * (34-2)/2 + 1 *18 =  3 * 6  *64

        #self.conv4=nn.Conv2d(64,64,5,stride=1,padding=1) #((10-5+2*1)/1)+1 * ((17-5+2*1)/1)+1 *64 =8 * 15 *64
        #self.act4=nn.LeakyReLU(0.1)
        #self.pool4=nn.MaxPool2d(kernel_size=2,stride=2) # stride=2   (8-2)/2 + 1  * (15-2)/2 + 1 *64 =  4 * 7  *64
        
        self.flat=nn.Flatten()

        self.fc_combo1=nn.Linear(in_features=1008,out_features=512,device=GPU)
        self.act_MLP1=nn.LeakyReLU(0.1)
        self.fc_combo2=nn.Linear(in_features=512,out_features=512,device=GPU)
        self.act_MLP2=nn.LeakyReLU(0.1)
        #self.fc_combo3=nn.Linear(in_features=1024,out_features=1024)
        #self.act_MLP3=nn.LeakyReLU(0.1)
        #self.fc_combo4=nn.Linear(in_features=1024,out_features=512)
...
list_of_Losses=[pde_cond_IBM_up,#1000
                           ...
                           pde_cond_y,#5000
                           pde_cond_mass]
solver = tp.solver.PIAN_Solver_CNN_Wasserstein(list_of_Losses,#1000
                               generator=model,
                               discriminator=disc,
...
class Data_set_pinn(torch.utils.data.Dataset):
    ...
    def __len__(self):
        return Data_Pinn.shape[0]

    def __getitem__(self,idx):
        #return self.x_train[idx] , self.y_train[idx]
        #print(torch.as_tensor([self.x_train[idx]]).shape)
        P_xtrain=self.Data_Pinn[idx]
        P_ytrain=self.Data_Pinn[idx]
...
trainer = pl.Trainer(gpus=1,# use one GPU
                     max_steps=500000, # iteration number
                     benchmark=True, # faster if input batch has constant size
                     logger=comet_logger, # for writting into tensorboard
                     log_every_n_steps=100,
                     enable_checkpointing=False,
                     limit_val_batches=0,
                        num_sanity_val_steps=0) # saving checkpoints ToDo: turn on checkpointing after first training phase
